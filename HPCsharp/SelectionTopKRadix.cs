...
// This file implements an in-place Top-K Radix Selection algorithm using the Most Significant Digit (MSD) approach for unsigned integers (uint).
...
namespace HPCsharp
{
    ...
    {
        // Move elements in the region to the right of the k-th bin (Rob) (i.e. greater than k-th bin) that belong in k-th bin into the k-th bin.
        // Swap if the element in the k-th bin belongs to the Rob region, otherwise just move the element that belongs in the k-th bin into that bin.
        // Generic implementation that for any digit size.
        private static int MoveRobElementsIntoKthBin(uint[] a, int startOfKthBin, int lengthOfKthBin, int startOfRob, int lengthOfRob, int shiftRightAmount, uint bitMask, int kthBin)
        {
            int endOfKthBin = startOfKthBin + lengthOfKthBin - 1;
            int endOfRob    = startOfRob    + lengthOfRob    - 1;
            int _current_rob = startOfRob, _current_ib = startOfKthBin; // _rob = right of k-th-bin, _ib = inside of bin
            while (true)
            {
                // Look for the element that belongs in the bin that k is in, to move into that bin
                for (; _current_rob <= endOfRob; _current_rob++)
                    if (((a[_current_rob] >> shiftRightAmount) & bitMask) == kthBin) break;
                // Look for the first location in the bin that k is in, which has an element that does not belong to the k-th bin
                if (_current_rob <= endOfRob)
                    for (; _current_ib <= endOfKthBin; _current_ib++)
                        if (((a[_current_ib] >> shiftRightAmount) & bitMask) != kthBin) break;

                if (_current_rob > endOfRob || _current_ib > endOfKthBin) break; // All the elements have been exhausted from either area
                if (((a[_current_ib] >> shiftRightAmount) & bitMask) > kthBin)
                    (a[_current_ib], a[_current_rob]) = (a[_current_rob++], a[_current_ib++]);  // swap
                else
                    a[_current_ib++] = a[_current_rob++];    // Move the element that belongs in the k-th bin into the bin
            }
            return _current_ib;
        }
        // Move elements in the k-th bin which belong in the region to the right of the k-th bin (Rob) (i.e. greater than k-th bin) into the Rob region.
        // Generic implementation that work for regions to the left or to the right of the k-th bin, and for any digit size.
        private static void MoveRobElementsOutOfKthBin(uint[] a, int startOfKthBin, int lengthOfKthBin, int startOfRob, int lengthOfRob, int shiftRightAmount, uint bitMask, int kthBin)
        {
            int endOfKthBin = startOfKthBin + lengthOfKthBin - 1;
            int endOfRob    = startOfRob    + lengthOfRob    - 1;
            int _current_rob = startOfRob, _current_ib = startOfKthBin; // _rob = right of k-th-bin, _ib = inside of bin
            while (true)
            {
                ...
                if (_current_rob > endOfRob || _current_ib > endOfKthBin) break; // All the element outside the bin have been exhausted or the bin that k is in is full or
                a[_current_rob++] = a[_current_ib++];   // Move the element that belongs to the right of the k-th bin into that region
            }
        }
        ...
        private static void RadixSelectionTopKWordInner(uint[] a, int first, int length, int shiftRightAmount, int k)
        {
            ...
            int[] count = HPCsharp.Algorithm.HistogramOneWordComponent(a, first, last, shiftRightAmount);

            int[] startOfBin = new int[PowerOfTwoRadix_loc + 1];
            ...
            // Determine which bin contains the k-th largest element. kthBin will hold the bin number.
            ...
            _current_ib = MoveRobElementsIntoKthBin( a, startOfBin[kthBin], startOfBin[kthBin + 1] - startOfBin[kthBin], startOfBin[kthBin + 1], last - startOfBin[kthBin + 1] + 1, shiftRightAmount, bitMask, kthBin);
            MoveRobElementsOutOfKthBin( a, _current_ib, startOfBin[kthBin + 1] - _current_ib, startOfBin[kthBin + 1], last - startOfBin[kthBin + 1] + 1, shiftRightAmount, bitMask, kthBin);
            ...
            if (shiftRightAmount > 0)          // end recursion when all the bits have been processes
            {
                ...
                // Only recurse into the bin that contains the k-th smallest element only if more than one element is in that bin
                if ((startOfBin[kthBin + 1] - startOfBin[kthBin]) > 1)
                    RadixSelectionTopKWordInner(a, startOfBin[kthBin], startOfBin[kthBin + 1] - startOfBin[kthBin], shiftRightAmount, k);
        ...
        /// <summary>
        /// In-place Selection of the top-k largest elements in an array.
        /// Processes one word-digits at a time, using MSD Radix-like algorithm.
        /// </summary>
        ...
        public static void SelectTopKRadix(this uint[] arrayToBeSelected, Int32 start, Int32 length, Int32 k)
        {
            ...
            RadixSelectionTopKWordInner(arrayToBeSelected, start, length, shiftRightAmount, k);
        }
        /// <summary>
        /// In-place Selection of the top-k element in an array.
        /// Processes one word-digits at a time, using MSD Radix-like algorithm.
        /// </summary>
        ...
        public static void SelectTopKRadix(this uint[] arrayToBeSelected, Int32 k)
        {
            ...
            const int Log2ofPowerOfTwoRadix_loc = 16;
            int shiftRightAmount = (sizeof(uint) / 2 * 16) - Log2ofPowerOfTwoRadix_loc;
            RadixSelectionTopKWordInner(arrayToBeSelected, 0, arrayToBeSelected.Length, shiftRightAmount, k);
        }
    }
}
