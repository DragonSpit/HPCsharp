...
namespace HPCsharp
{
    public static partial class Algorithm
    {
        ...
        public static int MedianOfMedians<T>(T[] arr, T[] arr_working, int start, int length, IComparer<T> comparer = null, int chunkSize = 5)
        {
            ...
            if (arr_working == null)
                throw new ArgumentNullException(nameof(arr_working));
            if (start < 0)
        ...
        /// <summary>
        /// Median of Medians computation - in-place.
        /// </summary>
        /// <param name="arr">array that is to be selected from in place</param>
        /// <param name="start">starting index of the subarray</param>
        /// <param name="length">length of the subarray</param>
        /// <param name="comparer">optional method to compare array elements</param>
        /// <returns>returns the index of an approximate median of the subarray</returns>
        public static int MedianOfMedians<T>(T[] arr, int start, int length, IComparer<T> comparer = null, int chunkSize = 5)
        {
            if (arr == null)
                throw new ArgumentNullException(nameof(arr));
            if (start < 0)
            ...
            //Console.WriteLine("MedianOfMediansInplace: arr[start] = {0}, arr[start+length] = {1}", arr[start], arr[start + length - 1]);

            int halfChunkSize = chunkSize / 2;
            int length_working = length;
            //Console.WriteLine("MedianOfMediansInplace: starting with chunkSize = {0}", chunkSize);
            while (length_working > 24)  // never let the last level have fewer than 5 elements to choose the median from
            {
                //Console.WriteLine("MedianOfMediansInplace: length_working = {0}", length_working);
                int numFullFiveTuples = length_working / chunkSize;
                for (int i = 0; i < numFullFiveTuples; i++)
                {
                    //Array.Sort(arr_working, start + i * chunkSize, chunkSize, comparer);
                    HPCsharp.Algorithm.InsertionSort<T>(arr, start + i * chunkSize, chunkSize, comparer);
                    (arr[start + i], arr[start + i * chunkSize + halfChunkSize]) = (arr[start + i * chunkSize + halfChunkSize], arr[start + i]); // Swap the median to the front
                }
                int remainingElements = length_working % chunkSize;
                if (remainingElements > 0)
                {
                    //Array.Sort(arr_working, start + numFullFiveTuples * chunkSize, remainingElements, comparer);
                    HPCsharp.Algorithm.InsertionSort<T>(arr, start + numFullFiveTuples * chunkSize, remainingElements, comparer);
                    (arr[start + numFullFiveTuples], arr[start + numFullFiveTuples * chunkSize + remainingElements / 2]) = (arr[start + numFullFiveTuples * chunkSize + remainingElements / 2], arr[start + numFullFiveTuples]); // Swap the median to the front
                }
                length_working = numFullFiveTuples + (remainingElements > 0 ? 1 : 0);
                // Console.WriteLine("MedianOfMedians: length = {0}, arr[start] = {1}, arr[start+length] = {2}", length_working, arr_working[start], arr_working[start + length_working - 1]);
            }
            HPCsharp.Algorithm.InsertionSort<T>(arr, start, length_working, comparer);
            arr[start] = arr[start + length_working / 2]; // Swap the median to the front
            //Console.WriteLine("MedianOfMedians: last length_working = {0}, median value = {1}", length_working, arr_working[start]);
            int j = 0;
            var equalityComparer = comparer ?? Comparer<T>.Default;
            for (; j < length; j++)
                if (equalityComparer.Compare(arr[start + j], arr[start]) == 0) break;
            if (j == length)
                throw new Exception("MedianOfMediansInplace: Cannot find the median of medians in the original array, which should never happen");
            //Console.WriteLine("MedianOfMedians: median of medians index = {0}", j);
            return start + j;
        }
        /// <summary>
        /// Partition for QuickSelect or QuickSort which chooses a median-of-medians as the pivot element - not-in-place.
        ...
        /// <summary>
        /// Partition for QuickSelect or QuickSort which chooses a median-of-medians as the pivot element - in-place.
        /// The input array is modified during the process.
        /// </summary>
        /// <param name="arr">source array</param>
        /// <param name="l">left index of the subarray, inclusive</param>
        ...
        /// <param name="comparer">optional method to compare array elements</param>
        /// <returns>returns the index of where the pivot element ended up at</returns>
        public static int PartitionMoM<T>(this T[] arr, int l, int r, IComparer<T> comparer = null, int chunkSize = 5)
        {
            var equalityComparer = comparer ?? Comparer<T>.Default;
            int i = l - 1;   // start index of smaller elements
            int j = r;       // start index of larger  elements
            int median_index = MedianOfMedians<T>(arr, l, r - l + 1, comparer, chunkSize);
            (arr[r], arr[median_index]) = (arr[median_index], arr[r]); // Move pivot to the last element location
            T v = arr[r];    // pivot is the last element
        ...
        private static void SelectMoMGenericNonRecursive_loc<T>(T[] arr, int l, int r, int k, IComparer<T> comparer = null, int chunkSize = 5)
        {
            //int count = 0;
            while (r > l)
            {
                int i = PartitionMoM(arr, l, r, comparer, chunkSize);
                //Console.WriteLine("SelectMoMGenericNonRecursive_loc: l = {0}, r = {1}, k = {2}, PartitionMoM = {3}", l, r, k, i);
                if (i >= k) r = i - 1;
                if (i <= k) l = i + 1;
                // count++;
        ...
        /// <summary>
        /// Selection using Median of Medians when choosing the pivot partition element. Not-in-place, linear time O(n) in the worst case.
        /// </summary>
        ...
        /// <summary>
        /// Selection using Median of Medians when choosing the pivot partition element - in-place. Linear time O(n) in the worst case.
        /// </summary>
        /// <param name="arrayToBeSelected">array that is to be selected from in place</param>
        /// <param name="start">starting index of the subarray</param>
        /// <param name="length">length of the subarray</param>
        ...
        /// <param name="comparer">optional method to compare array elements</param>
        /// <returns>returns the k-th element of the array</returns>
        public static T SelectMomInplace<T>(T[] arrayToBeSelected, Int32 start, Int32 length, Int32 k, IComparer<T> comparer = null, int chunkSize = 5)
        {
            if (arrayToBeSelected == null)
                throw new ArgumentNullException(nameof(arrayToBeSelected));
            if (start < 0)
            ...
            SelectMoMGenericNonRecursive_loc(arrayToBeSelected, start, start + length - 1, k, comparer, chunkSize);
            return arrayToBeSelected[k];
        }
        /// <summary>
        /// Selection using Median of Medians when choosing the pivot partition element - in-place. Linear time O(n) in the worst case.
        ...
        /// <param name="comparer">optional method to compare array elements</param>
        /// <returns>returns the k-th element of the array</returns>
        public static T SelectMomInplace<T>(T[] arrayToBeSelected, Int32 k, IComparer<T> comparer = null, int chunkSize = 5)
        {
            if (arrayToBeSelected == null)
                throw new ArgumentNullException(nameof(arrayToBeSelected));
            if (arrayToBeSelected.Length <= 0)
            ...
            SelectMoMGenericNonRecursive_loc(arrayToBeSelected, 0, arrayToBeSelected.Length - 1, k, comparer, chunkSize);
            return arrayToBeSelected[k];
        }
    }
}
