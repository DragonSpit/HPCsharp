...
namespace HPCsharp
{
    static public partial class Algorithm
    {
        ...
        // This method should be much better for arrays of objects/classes for JavaScript, Java, and C# and Python.
        ...
        private static uint[] SortRadix2(this uint[] inputArray)
        {
            ...
            uint[] outputArray = new uint[inputArray.Length];
            uint[] count = new uint[numberOfBins];
            uint[] count1 = new uint[numberOfBins];
            uint[] count2 = new uint[numberOfBins];
            uint[] count3 = new uint[numberOfBins];
            bool outputArrayHasResult = false;

            uint bitMask = 255;
            uint bitMask1 = 255 << 8;
            uint bitMask2 = 255 << 16;
            uint bitMask3 = (uint)255 << 24;
            int shiftRightAmount = 0;
            ...
            while (bitMask != 0)    // end processing digits when all the mask bits have been processed and shifted out, leaving no bits set in the bitMask
            {
                ...
                if (bitMask == 255)     // first pass
                    for (uint current = 0; current < inputArray.Length; current++)    // Scan Key array and count the number of times each digit value appears - i.e. size of each bin
                    {
                        count[ExtractDigit(inputArray[current], bitMask, 0)]++;
                        count1[ExtractDigit(inputArray[current], bitMask1, 8)]++;
                        count2[ExtractDigit(inputArray[current], bitMask2, 16)]++;
                        count3[ExtractDigit(inputArray[current], bitMask3, 24)]++;
                    }
                else if (bitMask == bitMask1)
                    count = count1;
                else if (bitMask == bitMask2)
                    count = count2;
                else if (bitMask == bitMask3)
                    count = count3;
                ...
                // TODO: Measure the time per pass of
                for (uint current = 0; current < inputArray.Length; current++)
                    outputArray[endOfBin[ExtractDigit(inputArray[current], bitMask, shiftRightAmount)]++] = inputArray[current];
        ...
        // Radix Sort implementation which performs a single pass over the input Array to determine bins for each of the digits, to reduce the number of passes.
        /// <summary>
        /// Sort an array of user defined class containing an unsigned integer Key, using Radix Sorting algorithm. Linear time sort algorithm.
        /// Faster algorithm that uses 8N bytes more temporary memory than SortRadix, where N is the size of the input array.
        /// </summary>
        ...
        /// <param name="getKey">user provided function to extract the unsigned 32-bit key sorted on, from the user defined class</param>
        /// <returns>sorted array of user defined class</returns>
        public static T[] SortRadix3<T>(this T[] inputArray, Func<T, UInt32> getKey)
        {
            int numberOfBins = 256;
            int Log2ofPowerOfTwoRadix = 8;
            T[] outputArray = new T[inputArray.Length];
            ...
            while (bitMask != 0)    // end processing digits when all the mask bits have been processed and shifted out, leaving no bits set in the bitMask
            {
                ...
                if (bitMask == 255)     // first pass
                    ...
                    {
                        inKeys[current] = getKey(inputArray[current]);
                        count[ExtractDigit(inKeys[current], bitMask, 0)]++;
                        count1[ExtractDigit(inKeys[current], bitMask1, 8)]++;
                        count2[ExtractDigit(inKeys[current], bitMask2, 16)]++;
                        count3[ExtractDigit(inKeys[current], bitMask3, 24)]++;
                    }
                else if (bitMask == bitMask1)
                    count = count1;
                else if (bitMask == bitMask2)
                ...
                // TODO: Even the bin computations could be done during the first pass
                startOfBin[0] = endOfBin[0] = 0;
                for (uint i = 1; i < numberOfBins; i++)
                    startOfBin[i] = endOfBin[i] = (uint)(startOfBin[i - 1] + count[i - 1]);
