...
namespace HPCsharp
{
	static public partial class Algorithm
	{
		...
		private static void heapSort(int[] src, int begin, int end)
		{
			int heapN = end - begin;
		...
		// insertion sort
		private static void insertionSort(int[] src, int left, int right)
		{
		...
		private static int findPivot(int[] src, int a1, int b1, int c1)
		{
		...
		// --> recursion level
		private static void sortDataUtil(int[] src, int begin, int end, int depthLimit)
		{
			if (end - begin > 16)
			{
				if (depthLimit == 0)
				...
				// partition
				sortDataUtil(src, begin, p - 1, depthLimit);
				sortDataUtil(src, p + 1, end,   depthLimit);
			}
			else
			{
		...
		public static void IntroSort(int[] src)
		{
			...
			sortDataUtil(src, 0, src.Length - 1, depthLimit);
		}
	}
}
